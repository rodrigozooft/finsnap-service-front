...
output "github_actions_role_arn" {
  description = "ARN of the GitHub Actions role (use for AWS_ROLE_ARN secret)"
...
output "github_secrets_configuration" {
  ...
  value       = <<-EOT
    ...
    - S3_BUCKET_DEV: ${module.s3.bucket_id}
    - CLOUDFRONT_DISTRIBUTION_ID_DEV: ${module.cloudfront.distribution_id}
